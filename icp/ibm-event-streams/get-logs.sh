...
for pod in $pods; do
    ...
    kubectl logs $pod access-controller -n $NAMESPACE > $logdir/$pod/accesscontroller.log
    kubectl logs $pod redis -n $NAMESPACE > $logdir/$pod/kubectl-redis.log
    echo -e "\033[0;32m [DONE]\033[0m"
done
...
for pod in $pods; do
    ...
    mkdir -p $logdir/$pod
    kubectl describe pod $pod -n $NAMESPACE > $logdir/$pod/pod-describe.log
    kubectl logs $pod -n $NAMESPACE > $logdir/$pod/elastic.log
    echo -e "\033[0;32m [DONE]\033[0m"
done
...
for pod in $pods; do
    ...
    mkdir -p $logdir/$pod
    kubectl describe pod $pod -n $NAMESPACE > $logdir/$pod/pod-describe.log
    kubectl logs $pod -n $NAMESPACE > $logdir/$pod/indexmgr.log
    echo -e "\033[0;32m [DONE]\033[0m"
done
...
for pod in $pods; do
    ...
    mkdir -p $logdir/$pod
    kubectl describe pod $pod -n $NAMESPACE > $logdir/$pod/pod-describe.log
    kubectl logs $pod kafka -n $NAMESPACE > $logdir/$pod/kafka.log
    kubectl logs $pod metrics-reporter -n $NAMESPACE > $logdir/$pod/metrics-reporter.log
    kubectl logs $pod metrics-proxy -n $NAMESPACE > $logdir/$pod/metrics-proxy.log
    kubectl logs $pod healthcheck -n $NAMESPACE > $logdir/$pod/healthcheck.log
    echo -e "\033[0;32m [DONE]\033[0m"
done
...
for pod in $pods; do
    ...
    mkdir -p $logdir/$pod
    kubectl describe pod $pod -n $NAMESPACE > $logdir/$pod/pod-describe.log
    kubectl logs $pod -n $NAMESPACE > $logdir/$pod/proxy.log
    echo -e "\033[0;32m [DONE]\033[0m"
done
...
for pod in $pods; do
    ...
    mkdir -p $logdir/$pod
    kubectl describe pod $pod -n $NAMESPACE > $logdir/$pod/pod-describe.log
    kubectl logs $pod rest -n $NAMESPACE > $logdir/$pod/rest.log
    kubectl logs $pod codegen -n $NAMESPACE > $logdir/$pod/codegen.log
    kubectl logs $pod proxy -n $NAMESPACE > $logdir/$pod/proxy.log
    echo -e "\033[0;32m [DONE]\033[0m"
done
...
for pod in $pods; do
    ...
    mkdir -p $logdir/$pod
    kubectl describe pod $pod -n $NAMESPACE > $logdir/$pod/pod-describe.log
    kubectl logs $pod ui -n $NAMESPACE > $logdir/$pod/ui.log
    kubectl logs $pod redis -n $NAMESPACE > $logdir/$pod/redis.log
    kubectl logs $pod proxy -n $NAMESPACE > $logdir/$pod/proxy.log
    echo -e "\033[0;32m [DONE]\033[0m"
done
...
for pod in $pods; do
    ...
    mkdir -p $logdir/$pod
    kubectl describe pod $pod -n $NAMESPACE > $logdir/$pod/pod-describe.log
    kubectl logs $pod -n $NAMESPACE > $logdir/$pod/zookeeper.log
    echo -e "\033[0;32m [DONE]\033[0m"
done
