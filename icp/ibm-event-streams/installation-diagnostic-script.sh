    ...
    then
       echo "User does not have the authority to view secrets. Unable to check OAuth configuration."
        ...
        then
            ...
            eventstreamsOIDCClientID=`echo -n $eventstreamsOIDCEncodedClientID| base64 -d`
            eventstreamsOIDCClientSecret=`kubectl get secret $RELEASENAME-ibm-es-oauth-secret -o=jsonpath='{.data.password}' -n$NAMESPACE | base64 -d`
            ...
            then
                ...
                oauthAdminToken=`echo $encryptedOAuthAdminToken | base64 -d`
...
getCurrentUserRole() {
    ...
    then
        # echo  $OIDC_BASIC_AUTH_TOKEN
        # echo $USER_ACCESS_TOKEN
        # echo https://$MASTER_IP:$CLUSTER_PORT/idprovider/v1/auth/introspect
        ...
        user=${user%%\"*}
        user=admin
        # echo https://$MASTER_IP:$CLUSTER_PORT/idmgmt/identity/api/v1/users/$user/getHighestRoleForCRN?crn=crn%3Av1%3Aicp%3Aprivate%3Aeventstreams%3A$CLUSTER_NAME%3An%2F$NAMESPACE%3Ar%2F$RELEASENAME%3A%3A
        USER_ROLE=`curl -k -s -X GET -H "Content-Type: application/json" -H "Accept: application/json" -H "Authorization: Bearer $USER_ACCESS_TOKEN" https://$MASTER_IP:$CLUSTER_PORT/idmgmt/identity/api/v1/users/$user/getHighestRoleForCRN?crn=crn%3Av1%3Aicp%3Aprivate%3Aeventstreams%3A$CLUSTER_NAME%3An%2F$NAMESPACE%3Ar%2F$RELEASENAME%3A%3A`
